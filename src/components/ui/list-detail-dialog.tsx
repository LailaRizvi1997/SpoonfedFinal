...
export function ListDetailDialog({ isOpen, onClose, listId, listName }: ListDetailDialogProps) {
  ...
  const loadListRestaurants = async () => {
    ...
    try {
      ...
      const loadedRestaurants = data.map((item: any) => item.restaurant)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl">
        ...
        <div className="space-y-4 pt-4">
          ...
          {/* Existing Restaurants in the List */}
          <div className="space-y-2">
            <h3 className="font-semibold">Restaurants in this List ({restaurants.length})</h3>
            {isLoading ? (
